...
import { loadKeys } from "./server/keys.js";
...
async function main(): Promise<number> {
  ...
  // Ahead of every command that follows, so `doctor` reports what `serve` will
  // actually see. A key saved in the app and one exported in this shell are the
  // same thing by the time anything below reads `process.env`.
  loadKeys();
  ...
  const server = serve({
    fetch: (request) => {
      ...
      // `/v1` is the gateway — the OpenAI and Anthropic surfaces other tools
      // speak. It has to reach Hono rather than the static file server, or
      // Claude Code's POST would be answered with index.html.
      const served = url.pathname.startsWith("/api") || url.pathname.startsWith("/v1");
      return served ? app.fetch(request) : servePage(url.pathname);
    },
    hostname: HOST,
    port: options.port,
  });
