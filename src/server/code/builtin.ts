...
export const builtinAdapter: CodingAgentAdapter = {
  ...
  async run(context: RunContext, writer: UIMessageStreamWriter): Promise<RunResult> {
    ...
    // Available at every level, including read-only, because generating an
    // image touches nothing on disk — it is the same shape of thing as
    // `remember`, not as `write_file`. `toolsForLevel` governs what may reach
    // the checkout, and this reaches a provider instead.
    ...
    const result = streamText({
      model,
      system,
      // Passed to the conversion as well, so `toModelOutput` strips generated
      // image bytes out of a resumed session's history. See `images/tools.ts`.
