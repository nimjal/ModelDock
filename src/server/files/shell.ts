 ...
 *
 * That covers three sets, and it has to cover all three. The variables the
 * store names are passed in as `secrets`. The conventional ones someone may
 * have exported without ever telling ModelDock are in `ALWAYS_STRIP`. And every
 * key saved through the app is stripped too — those are put into `process.env`
 * at startup precisely so the rest of the app cannot tell them apart from an
 * exported one, which would make them invisible to a filter that only knew
 * about the other two.
 *
...
import { savedNames } from "../keys.js";
 ...
 * ModelDock about them. Every provider on the setup screen contributes its
 * variable here, so adding a preset cannot quietly widen what a shell can read.
 */
const ALWAYS_STRIP = [
  "ANTHROPIC_API_KEY",
  "OPENAI_API_KEY",
  ...
  "GH_TOKEN",
  "NPM_TOKEN",
  ...PRESETS.map((preset) => preset.apiKeyEnv).filter((name): name is string => Boolean(name)),
];
...
export function childEnv(secrets: string[] = []): NodeJS.ProcessEnv {
  ...
  // Read from disk rather than from a cached list, so a key saved a moment ago
  // is already unreadable by the next command rather than by the next restart.
  for (const name of [...ALWAYS_STRIP, ...savedNames(), ...secrets]) {
    delete env[name];
  }
