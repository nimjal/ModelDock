...
export const SYNCED = [
  ...
  /**
   * Every column travels, and per-column merging is exactly what this row wants.
   *
   * The four settings on it are independent choices — a default model and an
   * image model have nothing to do with each other — so setting one on each of
   * two machines has to keep both. That is the ordinary merge rule doing its
   * job, and the reason this is a row with columns rather than a blob.
   *
   * Both connection ids are safe to send because `connections` syncs whole and
   * applies before this table does. There is no `directory`-shaped column here:
   * nothing on this row describes the machine it was written on.
   */
  synced(workspace, []),
] as const;
...
export const APPLY_ORDER: readonly string[] = [
  "connections",
  "coding_agents",
  // After `connections`, both of whose ids it points at.
  "workspace",
  "projects",
  "threads",
  "messages",
