 ...
 *
 * Nearest scope wins, as everywhere else in this codebase: the thread's own
 * connection, then the project default, then the workspace default, then
 * whatever single connection exists. The last fallback matters on first run —
 * someone who has added exactly one connection should be able to type
 * immediately without first choosing it from a menu.
 *
 * The workspace default is consulted here *as well as* being stamped onto a
 * thread at creation, which is not redundant. Stamping is what makes the berth
 * correct the instant a chat opens; this is what catches a thread whose
 * connection was deleted out from under it, which would otherwise fall all the
 * way through to an arbitrary first row rather than to the one someone chose.
 */
export async function connectionForThread(threadId: string) {
  ...
  candidates.push((await chatDefault(database)).connectionId);
