...
/**
 * The workspace row: a default model for new chats, and which connection draws.
 *
 * `CREATE TABLE IF NOT EXISTS` and nothing else. The row itself is not inserted
 * here, because a migration that writes a row would have to bypass
 * `db/write.ts` to do it and would leave no changelog entry — see
 * `workspace.ts`, which creates it on first read through the ordinary write
 * path so it syncs like anything else.
 *
 * Note this is *not* the `settings` table returning. See `schema.ts` for why
...
CREATE TABLE IF NOT EXISTS workspace (
  ...
  default_connection_id TEXT REFERENCES connections(id),
  default_model TEXT,
  image_connection_id TEXT REFERENCES connections(id),
  image_model TEXT,
  created_at INTEGER NOT NULL DEFAULT (unixepoch() * 1000),
  updated_at INTEGER NOT NULL DEFAULT (unixepoch() * 1000),
  deleted_at INTEGER
