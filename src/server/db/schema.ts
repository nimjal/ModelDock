...
/**
 * The choices that belong to the workspace rather than to any one thread.
 *
 * One row, always. Which is a shape this file has had before and removed: the
 * `settings` key/value bag was dropped in migration 4, and the note there is
 * worth answering rather than quietly contradicting. It was dropped because it
 * had no ULID id, no `createdAt` and no `deletedAt`, so it could not take part
 * in a sync at all — and because nothing read it. This table has all three, is
 * listed in `sync/tables.ts` like every other, and merges per column the same
 * way: set a default model on the desktop and an image model on the laptop and
 * both survive, because they were never in conflict.
 *
 * It is a table with columns rather than a bag with keys for the same reason
 * every other table here is: `defaultConnectionId` is a foreign key that the
 * database can enforce and the compiler can see, and `'default_connection_id'`
 * as a string in a `value` column is neither.
 *
 * The id is fixed for the reason `seed.ts` explains at length — two fresh
 * machines that pair should have one workspace row, not two that each think
 * they are the only one.
 *
 * Nothing here is required. Every column is nullable and the resolution order
 * that reads them falls through to what the app did before this table existed,
 * so a store that has never been told a default behaves exactly as it used to.
 */
export const workspace = sqliteTable("workspace", {
   ...
   *
   * Stamped onto the thread at creation rather than resolved at each turn, so
   * the berth is right the moment the chat opens and so changing the default
   * later does not silently re-point conversations someone already began. It is
   * *also* consulted as a fallback in `chat/turn.ts`, which is not redundant:
   * that path catches a thread whose connection was deleted out from under it.
   */
  defaultConnectionId: text("default_connection_id").references(() => connections.id),
  /** Null means "that connection's own model", exactly as on `threads`. */
  defaultModel: text("default_model"),
  /**
   * Which connection draws, and with what.
   *
   * Deliberately separate from the default above, because the model answering
   * and the model drawing are almost never the same one — Anthropic has no
   * image model at all, so a Claude conversation that can generate an image is
   * the normal case rather than the exception. `images/registry.ts` is what
   * turns this pair into something runnable.
   */
  imageConnectionId: text("image_connection_id").references(() => connections.id),
