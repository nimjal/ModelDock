...
import { imageEngine } from "../workspace.js";
...
chatRoutes.post("/chat", async (c) => {
  ...
  // Which connection draws, which is almost never the one answering. Null when
  // nothing is configured, in which case the tool is simply not offered rather
  // than offered and failing — the same treatment a project without a directory
  // gets from Cowork above.
  const drawing = await imageEngine(database);

  const tools = {
    ...memoryTools({ db: database, projectId: thread.projectId, threadId }),
    ...skillTools({ db: database, projectId: thread.projectId }),
    ...(root ? fileTools({ root }) : {}),
    ...(drawing ? imageTools({ engine: drawing }) : {}),
  };

  const result = streamText({
    model,
    system,
    // `tools` is passed here as well as to `streamText`, and it is load-bearing
    // rather than tidy: `toModelOutput` is applied during this conversion, and
    // it is the only thing keeping a megabyte of generated-image base64 out of
    // the context window every time a conversation with a picture in it is
    // replayed from the store. See the note in `images/tools.ts`.
    messages: await convertToModelMessages(toUiMessages(history), { tools }),
    tools,
