...
/** What `generate_image` returns. Rendered, never described. */
...
/**
 * A generated image, shown rather than reported.
 *
 * The one tool whose result is the point, so it gets the transcript's only
 * departure from "tool calls are a quiet line". Still no colour and still no
 * container: the image sits at the reading measure the way a figure would.
 *
 * Two or more come back as a row that wraps, because asking for variations and
 * being shown them stacked full-width defeats the reason for asking.
 */
function GeneratedImages({ part }: { part: Part }) {
  const output = part.output as ImageOutput | undefined;
  const input = part.input as { prompt?: string } | undefined;
  const prompt = output?.prompt ?? input?.prompt;

  if (output?.error) {
    return (
      <p className="mb-2 text-[0.8125rem]" style={{ color: "var(--ink-2)" }}>
        <span style={{ color: "var(--ink-3)" }}>Could not generate an image</span> — {output.error}
      </p>
    );
  }
  ...
  if (images.length === 0) {
    return (
      <p className="mb-2 text-[0.8125rem]" style={{ color: "var(--ink-3)" }}>
        Generating an image
  ...
  return (
    <figure className="mb-3">
      <div className="flex flex-wrap gap-2">
        {images.map((image) => (
          <img
            key={image.url.slice(-64)}
            src={image.url}
            alt={prompt ?? "Generated image"}
            className="max-h-[28rem] w-full max-w-full rounded-[var(--radius)] object-contain"
            style={{
              background: "var(--wash)",
              // Sized so a pair sits side by side and a single one does not
              // stretch past the measure the reply is set at.
              flex: images.length > 1 ? "1 1 16rem" : "0 1 auto",
            }}
          />
        ))}
      </div>
      {prompt && (
        <figcaption className="mt-1.5 text-[0.6875rem]" style={{ color: "var(--ink-3)" }}>
          {prompt}
        </figcaption>
      )}
    </figure>
...
function ToolCall({ part, onApprove }: { part: Part; onApprove?: Approve }) {
  ...
  if (name === "generate_image") return <GeneratedImages part={part} />;
