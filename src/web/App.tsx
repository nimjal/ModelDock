...
export function App() {
  ...
  // Open on the most recent conversation, or on setup if there is nothing to
  // reach yet — the one screen that can actually be acted on.
  //
  // Setup is shown only while it is the answer. Once a connection works, or
  // once it has been dismissed, this lands on a conversation like any other
  // start; the screen stays reachable from Connections rather than becoming a
  // permanent reminder that you once had nothing configured.
  //
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const dismissed = localStorage.getItem("modeldock.welcomed") === "yes";

      if (!ready && !dismissed) {
        setSurface({ kind: "welcome" });
      } else if (threadData.threads[0]) {
  ...
  /** Leaving setup, either finished or skipped. Both mean "don't open here again". */
  const leaveWelcome = async () => {
    localStorage.setItem("modeldock.welcomed", "yes");
    const threads = await refresh();
    if (threads[0]) {
      setSurface({ kind: "thread", id: threads[0].id });
      return;
    }
    const { thread } = await api.createThread({});
    await refresh();
  ...
  /**
   * The swap. One or two columns change; the conversation is untouched.
   *
   * `model` absent means "this provider, its own default", which is what
   * picking a connection should do — carrying the old provider's model over to
   * a new one would name something that does not exist there. Passing one
   * changes only the model, so the two halves of the berth stay independent.
   *
   ...
   */
  const dock = async (connectionId: string, model?: string) => {
    if (activeThread) {
      await api.updateThread(activeThread.id, { connectionId, model: model ?? null });
      await refresh();
      return;
    }
    const { thread } = await api.createThread({ connectionId });
    if (model) await api.updateThread(thread.id, { model });
    await refresh();
    setSurface({ kind: "thread", id: thread.id });
  };
        ...
        {booted && surface.kind === "thread" ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            {surface.kind === "welcome" && (
              <Welcome onChanged={() => void refresh()} onDone={() => void leaveWelcome()} />
            )}
