...
describe("threads", () => {
  it("creates a chat with no project, and asks for nothing", async () => {
    // The inversion the rewrite is for: this must not require setup.
    const response = await app.request(`${BASE}/api/threads`, json({}));
    expect(response.status).toBe(201);

    const body = (await response.json()) as {
      thread: { projectId: null; connectionId: string | null };
    };
    expect(body.thread.projectId).toBeNull();
  });

  /**
   * A new chat arrives already docked.
   *
   * This used to assert `connectionId` was null, which was a statement about
   * the mechanism rather than about the guarantee. The guarantee — that
   * starting a conversation requires no setup — is asserted above and is
   * unchanged. What is new is that the thread is stamped with the workspace
   * default at creation, so the berth names the right engine the moment the
   * chat opens instead of reading "No connection" until the first message
   * resolves one. See the note on `POST /threads`.
   */
  it("stamps a new chat with the default connection", async () => {
    const [connection] = await db()
      .insert(connections)
      .values({ name: "Default Engine", kind: "anthropic", model: "claude-sonnet-4-5" })
      .returning();

    await app.request(`${BASE}/api/workspace`, {
      method: "PATCH",
      headers: HEADERS,
      body: JSON.stringify({
    ...
    const response = await app.request(`${BASE}/api/threads`, json({}));
    const body = (await response.json()) as {
      thread: { connectionId: string | null; model: string | null };
    };

    expect(body.thread.connectionId).toBe(connection!.id);
    expect(body.thread.model).toBe("claude-opus-4-1");
  });

  it("lets an explicit connection win over the default", async () => {
    const [other] = await db()
      .insert(connections)
      .values({ name: "Chosen Explicitly", kind: "openai", model: "gpt-4.1" })
      .returning();

    const response = await app.request(`${BASE}/api/threads`, json({ connectionId: other!.id }));
    const body = (await response.json()) as { thread: { connectionId: string | null } };

    expect(body.thread.connectionId).toBe(other!.id);
  });
...
describe("chat preconditions", () => {
  it("explains that no connection is set up rather than failing obscurely", async () => {
    // Soft-deleted rather than removed, which is both what the app actually
    // does everywhere and the only thing that works now that a thread carries
    // a real foreign key to the connection it was stamped with — a hard
    // `DELETE FROM connections` is refused by the database, correctly.
    await db().update(connections).set({ deletedAt: Date.now() });
