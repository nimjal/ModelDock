...
reads correctly long after the model that produced it is gone. Pick a default in Settings
and every new conversation opens on it; changing that default later leaves the chats you
have already started where they are.

**Images** — point Settings at a connection that can draw and the assistant gains a
`generate_image` tool it can call mid-conversation. It is deliberately not tied to the
model answering, because those are rarely the same thing: Anthropic publishes no image
model at all, so Claude holding the conversation while OpenAI or a local endpoint makes
the picture is the *ordinary* arrangement here, not a workaround. Same key, or a separate
one, or a box on your own network — it is another connection, chosen the same way.
...
## Lending your models to your other tools

The store is not the only thing worth sharing. ModelDock also serves **every model it can
reach** on one local endpoint, in both of the protocols the world actually speaks:

```
http://127.0.0.1:8765/v1/models             every model, from every connection
http://127.0.0.1:8765/v1/chat/completions   OpenAI-shaped
http://127.0.0.1:8765/v1/messages           Anthropic-shaped
```

Models are named `<connection>/<model>` — `anthropic/claude-sonnet-4-5`,
`ollama/llama3.2`, `openrouter/anthropic/claude-sonnet-4-5`. The split is on the first
slash, so a provider whose own ids contain slashes still works.

Which means Claude Code can run on Gemini, or on a local Llama, without knowing anything
has changed. **Connect your tools** in Settings does that setup for you: it shows the
exact diff it would write to `~/.claude/settings.json` or
`~/.config/opencode/opencode.json`, writes it only when you say so, keeps a `.modeldock.bak`
copy of the original, and offers a revert that removes exactly what it added and nothing
else. A config file it cannot parse is reported rather than rewritten.

This is the inverse of the Code surface above, and both directions are the same claim:
there, ModelDock drives Claude Code and owns the transcript; here, Claude Code stays in
charge and ModelDock is merely the model. The engine is a detail either way.

Two things worth knowing. Tools are **declared but never executed** by the gateway — the
caller owns its own agentic loop, and running someone else's agent's file writes here
would mean doing it with no permission level and nothing to approve. And unlike the rest
of the API, this endpoint asks for a token, kept in `~/.modeldock/gateway.token`: loopback
plus a Host check is a fine defence against a web page, but it cannot tell one local
process from another, and this is the one surface where that distinction is worth money.
...
`providers/`, `images/` and `code/` are deliberately the same shape: a `catalog.ts` of pure
metadata and a `registry.ts` that turns a database row into something runnable. Adding a
provider or an engine means adding to a catalog and a switch, and the exhaustiveness checks
make the compiler tell you what else to touch. `images/registry.ts` is the clearest example
of what that buys — Anthropic's arm throws a sentence explaining that no image model exists
rather than falling into a default, so "cannot" stays distinct from "not thought about yet".
...
connection kinds, a default model for new chats, image generation as a tool, memory,
projects, Cowork, Skills, Code on the built-in engine or an external agent, per-call
approval, the local model gateway and the one-click handoff to Claude Code and OpenCode,
device-to-device sync, the MCP server, `doctor` and `npx modeldock`.
...
**Generated images are stored inline**, as a data URL in the message that produced them.
That keeps the promise at the top of this file exactly true — everything really is in the
one SQLite file, and images sync with no extra machinery — but it is a real cost, not a
free one: a few megabytes per image in the database, and again in every sync push of that
row for the rest of its life. It is the right default for a workspace of a few hundred
conversations and the wrong one for a few thousand images. Their bytes never reach a model's
context window, which is a separate problem and is solved.
