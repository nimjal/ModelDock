...
- Node 22 or later — `nvm use` picks the right line up from `.nvmrc`. `ai`,
  the `@ai-sdk/*` packages and `concurrently` all declare `node >=22`
  themselves, so 20 is out of scope for the dependency tree even though the
  store itself still builds on it. Prefer a current 22.x: two transitive
  packages want `>=22.13`, so the very earliest 22 releases install with
  warnings.
